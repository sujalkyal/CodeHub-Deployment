...
async function getTestCasesFromS3(slug) {
  ...
  console.log(
    `[Run API] Fetching test cases from S3: s3://${bucketName}/${key}`
  );
  try {
    ...
    if (error.name === "NoSuchKey") {
      console.warn(
        `[Run API] Test case file not found in S3 for slug: ${slug}`
      );
      return [];
    }
...
export async function POST(req) {
  try {
    ...
    console.log(
      `[Run API] Fetching boilerplate for problemId: ${problem.id}, languageId: ${languageId}`
    );
    ...
    if (!boilerplate || !boilerplate.fullcode || !boilerplate.code) {
      console.warn(
        `[Run API] Boilerplate not found for problem: ${problemSlug}, language ID: ${languageId}`
      );
      return NextResponse.json(
    ...
    console.log(
      `[Run API] Fetching test cases for problemSlug: ${problemSlug}`
    );
    ...
    if (sampleTestCases.length === 0) {
      console.warn(
        `[Run API] No sample test cases found for problemSlug: ${problemSlug}`
      );
      return NextResponse.json(
    ...
    console.log(
      `[Run API] Creating submission record for userId: ${userId}, problemId: ${problem.id}, languageId: ${languageId}`
    );
    ...
    for (const testCase of sampleTestCases) {
      console.log(
        `[Run API] Creating submissionTestCaseResults for submissionId: ${runSession.id}`
      );
      ...
      console.log(
        `[Run API] Created submissionTestCaseResults with id: ${resultRecord.id}`
      );
      testCaseMap.push({
    ...
    Promise.all(judge0Promises)
      .then(() =>
        console.log("[Run API] All Judge0 jobs dispatched successfully.")
      )
      .catch((err) =>
        console.error("[Run API] Error dispatching to Judge0:", err)
      );

    // --- Return runId and test case map ---
    console.log(
      `[Run API] Returning runId: ${runSession.id} and ${testCaseMap.length} test cases`
    );
    return NextResponse.json(
      {
        runId: runSession.id,
