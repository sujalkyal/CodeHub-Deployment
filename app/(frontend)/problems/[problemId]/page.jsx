    ...
    <div className="bg-slate-800/50 rounded-lg border border-slate-700 mb-3">
      ...
      >
        <div className="flex items-center gap-3">
          {statusInfo ? (
            <statusInfo.Icon className={`${statusInfo.color} w-5 h-5`} />
          ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-sm">
            {testCase.input && (
              <div>
                ...
                <pre className="text-slate-300 whitespace-pre-wrap bg-slate-800 p-3 rounded">
                  {testCase.input}
                </pre>
              </div>
            )}
            ...
            {result?.stderr && (
              <div className="md:col-span-2">
                ...
                <pre className="text-yellow-300 whitespace-pre-wrap bg-slate-800 p-3 rounded">
          ...
          <div className="flex items-center gap-6 mt-4 pt-4 border-t border-slate-700/50">
            {result?.time && (
              <div className="flex items-center gap-2 text-sm text-slate-400">
                <Clock size={14} /> Time: {result.time}s
              </div>
            )}
            {result?.memory && (
              <div className="flex items-center gap-2 text-sm text-slate-400">
                <Server size={14} /> Memory: {result.memory} KB
              </div>
            )}
          </div>
        </div>
...
export default function ProblemSolvePage() {
    ...
    axios
      ...
      .then((res) => {
        ...
        const defaultBoiler = res.data.boilerplates.find(
          (b) =>
            b.language.name.toLowerCase() === LANGUAGES[0].name.toLowerCase()
        );
  ...
  useEffect(() => {
    ...
    setCode(
      boiler?.code || `// Boilerplate for ${selectedLang.name} not found.`
    );
  ...
  const handleRun = async () => {
    ...
    try {
      const langBoiler = problem.boilerplates.find(
        (b) => b.language.id === selectedLang.id
      );
                ...
                <div className="flex items-center justify-between p-3 bg-slate-900 border-b border-slate-700">
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      className="flex items-center gap-2 bg-slate-700 text-white px-4 py-1.5 rounded-md font-semibold hover:bg-slate-600 transition disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-1.5 rounded-md font-bold hover:bg-indigo-500 transition disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                      {isSubmitting && (
                ...
                <div className="flex-grow p-5 overflow-y-auto">
                  ...
                  {runResult && (
                    <div>
                      ...
                      {runTestCases?.map((tc) => {
                        const resultForCase = runResult.results?.find(
                          (r) => r.id === tc.submissionTestCaseResultsId
                        );
                        return (
                          <TestCaseResult
                            key={tc.submissionTestCaseResultsId}
                            testCase={{ ...tc, isSample: true }}
                            result={resultForCase}
                          />
                        );
                      })}
                    </div>
